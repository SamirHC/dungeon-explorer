...
def get_sheet(sheet: pygame.Surface, palette: list[pygame.Color]) -> pygame.Surface:
...
def get_animation(
    sheet: pygame.Surface, palette: list[pygame.Color], size: tuple[int, int]
) -> Animation:
    W, H = size
...
anims = {
    (stat, anim_name): get_animation(sheet, palette, sizes)
    for stat, palette in pairings
    for anim_name, sheet, sizes in sheet_pairings
}


def load(stat: str, anim_id: int) -> Animation:
    return anims[stat, anim_id]
