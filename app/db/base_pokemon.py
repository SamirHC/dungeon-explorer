...
def load(poke_id: int):
    ...
    pokedex_number, name, category, primary_type, secondary_type, iq_group, movement_type = _cursor.execute(
    ...
    stats_growth = StatsGrowth(*zip(*_cursor.execute(
    ...
    level_up_moves = LevelUpMoves(*zip(*_cursor.execute(
    ...
    egg_moves = tuple(_cursor.execute(
        "SELECT move_id FROM egg_moves WHERE poke_id = ?", (poke_id, )
    ))
    hm_tm_moves = tuple(_cursor.execute(
