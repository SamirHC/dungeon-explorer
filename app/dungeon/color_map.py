...
class ColorMap:
    ...
    def transform_surface_ip(self, surface: pygame.Surface):
        surface.set_palette(
            [self.transform_color(color) for color in surface.get_palette()]
        )
    ...
    def transform_palette_animation(self, anim: PaletteAnimation) -> PaletteAnimation:
        ...
        new_frames = [
            [self.transform_color(col) for col in palette] for palette in frames
        ]
        return PaletteAnimation(new_frames, durations)

    def transform_surface_colors(
        self, surface: pygame.Surface, colors: list[pygame.Color]
    ):
